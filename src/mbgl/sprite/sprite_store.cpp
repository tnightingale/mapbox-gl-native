...
void SpriteStore::setURL(const std::string& url) {
    ...
    Resource jsonResource = { Resource::Kind::SpriteJSON, jsonURL };
    FileSource* jsonFS = util::ThreadContext::getFileSourceHandlingResource(jsonResource);
    loader->jsonRequest = jsonFS->request(jsonResource, [this, jsonURL](Response res) {
        if (res.stale) {
    ...
    Resource imageResource = { Resource::Kind::SpriteImage, spriteURL };
    FileSource* imageFS = util::ThreadContext::getFileSourceHandlingResource(imageResource);
    loader->spriteRequest = imageFS->request(imageResource, [this, spriteURL](Response res) {
        if (res.stale) {
            // Only handle fresh responses.
            return;
        }
